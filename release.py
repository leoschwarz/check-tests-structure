...
def get_remote_tags() -> list[str]:
    ...
    out = subprocess.run(
        ["git", "ls-remote", "--tags", "origin"], check=True, capture_output=True
    )
    return [
...
def get_most_recent_release(remote: bool) -> str:
    sorted_releases = sorted(
        get_existing_releases(remote=remote),
...
def check_version() -> str:
    ...
    if released_remote == current:
        print(
            f"Version {current} is already released remotely. Please bump the version in pyproject.toml"
        )
        sys.exit(1)
    elif released_local == current:
        print(
            f"Version {current} is already released locally. Please bump the version in pyproject.toml"
        )
        sys.exit(1)
    else:
        return current
